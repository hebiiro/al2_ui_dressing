...
namespace my
{
	...
	struct Window2 : Window
	{
		...
		virtual LRESULT on_wnd_proc(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param)
		{
			...
			{
				if (pair.second->on_wnd_proc(this, hwnd, message, w_param, l_param, &lr))
					return lr;
			}
		...
		struct Extension
		{
			...
			virtual BOOL on_wnd_proc(Window2* window, HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param, LRESULT* lr) = 0;
		};
