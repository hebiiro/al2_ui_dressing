...
namespace my
{
	...
	struct WindowInterceptor
	{
		...
		//
		virtual LRESULT on_wnd_proc(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param)
		{
			switch (message)
			{
			case WM_NCDESTROY:
				{
					auto lr = ::DefSubclassProc(hwnd, message, w_param, l_param);
					post_nc_destroy();
					return lr;
				}
			}
		...
		{
//			MY_TRACE_FUNC("{/hex}, {/hex}, {/hex}, {/hex}, {/hex}, {/hex}", hwnd, message, w_param, l_param, id, ref_data);

			auto window = (WindowInterceptor*)ref_data;
			return window->on_wnd_proc(hwnd, message, w_param, l_param);
		}
	...
	public:
		...
		thread_local inline static struct Associator
		{
			...
			// 内部的に使用されます。
			//
			inline static LRESULT CALLBACK hook_proc(int code, WPARAM w_param, LPARAM l_param)
			{
//				MY_TRACE_FUNC("{/}, {/hex}, {/hex}", code, w_param, l_param);

				if (code == HCBT_CREATEWND)
				{
					associator.subclass((HWND)w_param);
					associator.stop();
				}
