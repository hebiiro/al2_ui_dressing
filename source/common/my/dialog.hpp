...
namespace my
{
	struct Dialog : Window
	{
		...
		virtual BOOL create(HINSTANCE instance, LPCTSTR name, HWND parent, LPARAM l_param = 0)
		{
			Hooker hooker(*this);
			return !!::CreateDialogParam(instance, name, parent, dlg_proc, l_param);
		}
		...
		virtual INT_PTR on_dlg_proc(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param)
		{
			switch (message)
			{
			case WM_COMMAND:
		...
		static INT_PTR CALLBACK dlg_proc(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param)
		{
			auto dialog = static_cast<Dialog*>(from_handle(hwnd));
			if (!dialog) return FALSE;
			return dialog->on_dlg_proc(hwnd, message, w_param, l_param);
		}
